...
с [trackmania.io](https://trackmania.io). На странице знаков есть **Sign
Studio** — конструктор кастомного фона 2048×512: юзер выбирает стиль градиента
и цвета, превью рендерится в браузере на `<canvas>`, результат скачивается
JPEG'ом (для использования в игре картинку нужно захостить — например, на
dashmap.live). Админка (`/admin`) — управление
