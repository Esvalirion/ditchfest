...
с [trackmania.io](https://trackmania.io). Есть **Sign Studio** — отдельная
страница `/studio` (вход с `/signs`) с конструктором кастомного фона для знаков
в форматах 1×1 / 2×1 / 4×1 / 6×1 (512²…3072×512): юзер выбирает формат, стиль
градиента и цвета, превью рендерится в браузере на `<canvas>`, результат
скачивается JPEG'ом (для использования в игре картинку нужно захостить —
например, на dashmap.live). Админка (`/admin`) — управление
