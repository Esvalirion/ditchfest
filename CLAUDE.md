...
## Sign Studio (`client/src/components/SignStudio.vue`, `client/src/utils/gradientRenderer.js`, `client/src/data/signStudioGradients.js`)

Клиентский «конструктор фонов» 2048×512 — вкладка **Studio** внутри
`/signs` (в `SignsView.vue` она рендерится вместо галереи знаков, когда
`topFilter === 'studio'`; фильтры/аккордеон каталога в этом режиме скрыты).
Юзер выбирает стиль градиента и цвета, превью рисуется на `<canvas>` в
браузере, кнопка **Download** выгружает JPEG через `canvas.toBlob`. Сервер
не участвует; для использования в Trackmania картинку нужно захостить
внешне (например, на dashmap.live).
...
рисуется всегда последней поверх любых цветов, чтобы бренд читался).
Dots и base включаются/выключаются чекбоксами, по умолчанию оба включены.
...
`gradientRenderer.js` читает `options.kind` и `options.colors` (map по
ключам стопов) и вызывает `renderXxxGradient(ctx, W, H, colors)` для
нужного стиля. Добавление нового стиля = (1) запись в `GRADIENTS` и
(2) ветка `case` + функция `renderXxxGradient` в рендерере — **без правок
в `SignStudio.vue`**. Сейчас два стиля: `arrow` (стрелка-трафарет по маске
`bckgrnask.png`, две зоны × по два цвета) и `linear` (простой 3-стоповый
left→right).

**Стиль `arrow`** — stencil-композит: бинарная маска `bckgrnask.png`
(2048×512, white vs near-black, форма `>`) делит canvas на две половины.
Каждая зона получает свой left→right 2-стоповый градиент, рассчитанный на
своей половине canvas (не на весь canvas) — иначе юзер видел бы только
половину каждого градиента. Композит идёт попиксельно через `ImageData`:
для каждого пикселя — интерполяция между цветом правой зоны (маска white)
и левой (маска black), граница 110–150 smoothstep против бритвенного края.
...
ассета менять код не нужно.

**Производительность**: композит `arrow` ходит по ~1М пикселей за рендер,
поэтому `paint()` дебаунсится (60мс) — частые движения color-picker'а не
съедают кадр. Канвас — фиксированные 2048×512 в пикселях (экспорт 1:1),
CSS `aspect-ratio` масштабирует превью под ширину контейнера.
