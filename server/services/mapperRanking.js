...
/** Mappers ranked by total "+" across all their maps (highest first). Ties
 *  break by name — the array index (1-based) is the rank, not a SQL window
 *  function, matching the real tm-votes source exactly.
 *
 ...
 *  voting for the same map still only count once. */
async function getMapperResults() {
  ...
  `);
  return rows.map((r) => ({ accountId: r.account_id, name: r.name, votes: r.votes }));
}
