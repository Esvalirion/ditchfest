 ...
 *  AND any admin-added co-authors (map_coauthors). So a collaboration's
 *  votes and map-count land on each co-author too. The
...
async function getMapperResults() {
  const rows = (await pool.query(rankingQuery())).rows;
...
/** The ranking query. map_authors gathers every credited author: the primary
 *  author_account_id plus any admin-added co-authors (map_coauthors). */
